...
- **The neutral ramp is COOL LIGHT GREY, never warm.** Three steps by lightness: `--ds-paper` (page) → `--ds-paper-2` (sunken: wells, skeletons, hover) → `--ds-paper-3` (deep well: empty media, soft separators). Depth comes from the **weight** of the grey, never from warmth or a shadow. There is no `paper-warm`. Text greys (`--ds-text-muted`, `--ds-text-subtle`) sit on the same cool hue so copy never reads olive. A muted label on ink or purple uses `text-on-dark-muted` / `on-purple-muted` — never a raw hex. 
