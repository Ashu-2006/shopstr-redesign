...
export default function Payout() {
  ...
  const Opt = ({ value, icon, title, sub, rec }: { value: "shopstr" | "lightning"; icon: string; title: string; sub: string; rec?: boolean }) => (
    <button
    ...
    >
      <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-[10px] text-lg ${value === "shopstr" ? "bg-green text-ink" : "border-2 border-current"}`}>{icon}</span>
      <span>
    ...
    <>
      ...
      <OneWayFrame tone="purple" step="Payout" sticker="shape-sunstar-yellow" closeTo="/wallet">
        ...
        <div className="mt-4 flex flex-col gap-2.5">
          <Opt value="shopstr" icon="⚡" title="Keep them in Shopstr" sub="Land in your in-app wallet. Spend on the market with one tap." rec />
          <Opt value="lightning" icon="↗" title="Send to my Lightning wallet" sub="Auto-melt to an external wallet on every sale." />
        </div>
